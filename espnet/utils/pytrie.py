...
def query_match_results(trie, query, hot_idx2score):
    length = len(query)
    if length == 0:
        return None, 0
    start_idx = 0
    ...
    score = 0
    not_matched = True
    while start_idx < length and not_matched:
        if end_idx > length:
            bias_score = sum([hot_idx2score[i] for i in seq[start_idx: end_idx]])
            score += bias_score
            break
        if start_idx >= length:
            break
        #print('start idx: {}, end idx {}'.format(start_idx, end_idx))
        seq = query[start_idx: end_idx]
        if isinstance(seq, torch.Tensor):
            seq = seq.cpu().numpy().tolist() if seq.is_cuda else seq.numpy().tolist()
        res = trie.items(prefix=seq)
        #print('res: {}'.format(res))
        if res:
            if complete_match(res, seq):
                not_matched = False
                start_idx = end_idx
                end_idx = end_idx + 1
                bias_score = sum([hot_idx2score[i] for i in seq])
                score += bias_score
            else:
                end_idx += 1
        else:
    ...
    if start_idx < length:
        return start_idx, score
    else:
        return None, score


def complete_match(match_results, query):
    if isinstance(query, list):
        query = tuple(query)
    for key, _ in match_results:
        if key == query:
            return True
    return False
